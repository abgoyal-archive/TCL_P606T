...
#define MAX_FBLOCK_NUM 3
static struct lock_class_key fb_lock_key[MAX_FBLOCK_NUM];
static int fb_lock_key_num = 0;
...
static int do_register_framebuffer(struct fb_info *fb_info)
{
    ...
    /*add for lock prove*/
    if(fb_lock_key_num < MAX_FBLOCK_NUM){
        lockdep_set_class(&fb_info->lock, &fb_lock_key[fb_lock_key_num++]);
    }else{
        printk(KERN_WARNING "Already init framebuffer > 3 times, %d\n", fb_lock_key_num++);
        dump_stack();
    }
