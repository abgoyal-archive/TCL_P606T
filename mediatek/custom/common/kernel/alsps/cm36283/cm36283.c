...
static int cal_params[5] = {0};//max, mean, high, loww
...
static ssize_t cm36283_show_status(struct device_driver *ddri, char *buf)
{
	...
	len += snprintf(buf+len, PAGE_SIZE-len, "REGS: %02X %02X %02X %02lX %02lX\n", 
				...
				atomic_read(&cm36283_obj->ps_thd_val_high),cm36283_obj->ps_thd_val_low, cm36283_obj->enable);
...
static void cm36283_prox_calibration(struct i2c_client *client)
{
	...
	if(prox_mean > 100 || 0 == prox_mean ||  prox_mean>30&&prox_mean <35) 
	{   
		atomic_set(&obj->ps_thd_val_high, obj->hw->ps_threshold_high);
		atomic_set(&obj->ps_thd_val_low, obj->hw->ps_threshold_low);
	}
	else if(prox_mean>20&&prox_mean<30)
	{
		atomic_set(&obj->ps_thd_val_high, obj->hw->ps_threshold_high-10);
		atomic_set(&obj->ps_thd_val_low, obj->hw->ps_threshold_low-10);
	}
	else if(prox_mean<20)
	{
		atomic_set(&obj->ps_thd_val_high, obj->hw->ps_threshold_high-20);
		atomic_set(&obj->ps_thd_val_low, obj->hw->ps_threshold_low-20);	
	}
	else
	{   
	...
	//disable ps
#if JOHN_DEBUG
	cal_params[0] = atomic_read(&obj->ps_thd_val_high);     
	cal_params[1] = atomic_read(&obj->ps_thd_val_low);        
...
static int cm36283_init_client(struct i2c_client *client)
{
	...
	databuf[0] = CM36283_REG_PS_CONF3_MS;
	databuf[1] = 0x10;
	databuf[2] = 0x00;//JOHN MODIFY need to confirm interrupt mode PS_MS mode whether to set
	res = CM36283_i2c_master_operate(client, databuf, 0x3, I2C_FLAG_WRITE);
	if(res <= 0)
	{
		APS_ERR("i2c_master_send function err\n");
